...
async def test_create_user(client, get_user_from_database):
    user_data = {"name": "Arman", "surname": "Shamalganov", "email": "ashamalganov@gmail.com"}
...
async def test_create_user_duplicate_email_error(client, get_user_from_database):
    user_data = {"name": "Arman", "surname": "Shamalganov", "email": "ashamalganov@gmail.com"}
    user_data_same = {"name": "Mike", "surname": "Krivcov", "email": "mikeK@gmail.com"}
    resp = client.post("/user/", data=json.dumps(user_data))
    data_from_resp = resp.json()
    assert resp.status_code == 200
    assert data_from_resp["name"] == user_data["name"]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"name": "Arman", "surname": 456, "email": "lol"},
            422,
            {"detail": "Surname should contains only letters"},
        ),
        (
            {"name": "Arman", "surname": "Shamalganov", "email": "ashamalganov"},
            422,
            {
                "detail": [
                    {
